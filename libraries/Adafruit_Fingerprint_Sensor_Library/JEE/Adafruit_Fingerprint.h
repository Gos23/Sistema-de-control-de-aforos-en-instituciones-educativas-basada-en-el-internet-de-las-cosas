...
#define FINGERPRINT_DOWNIMAGE 0x0A
...
class Adafruit_Fingerprint {
 public:
  ...
  uint8_t matchUpload(void);
  uint8_t deleteModel(uint16_t id);
  uint8_t ReadSysPara(void);
  uint8_t SetSysPara(uint8_t parameter, uint8_t content);
    void setPara(uint32_t addr, uint8_t packettype, uint16_t len, uint8_t instruct, uint8_t para, uint8_t content);
  uint8_t uploadModel(uint8_t pkt2[], uint8_t pkt3[], uint8_t pkt4[], uint8_t pkt5[], uint8_t pkt6[], uint8_t pkt7[]);
  uint8_t uploadModelPercobaan(uint8_t pkt2[], uint8_t pkt3[], uint8_t pkt4[], uint8_t pkt5[]);
